...
class database_startup():
    def fullCreate(self):
        ...
        try: self.mysql_db_create()
            ...
            try: Database.create_block(Database())
                ...
                print(f"{SUCCESS}Completed full MySQL Database creation.{ENDC}\n") # Yippee!
    ...
    def mysql_tables_create(self):
        ...
        try: db = mysql.connector.connect(host=HB_HOST,user=DB_USER,passwd=DB_PASS,database=DB_NAME)
            ...
            cursor = db.cursor()
            cursor.execute("CREATE TABLE IF NOT EXISTS users (uuid VARCHAR(20) NOT NULL PRIMARY KEY, balance DECIMAL(18,6) UNSIGNED NOT NULL, blocks MEDIUMINT UNSIGNED NOT NULL, pooling BOOL NOT NULL, username VARCHAR(20) NOT NULL, UNIQUE(uuid))")
